...
export class ParseService {
    ...
    async parse_excel_file(
        file_url: Files /** prisma */,
    ) {
        const file =
            await this.prisma.files.findFirstOrThrow(
                {
                    where: {
                        id: file_url.id,
                    },
        ...
        for (
            ...
            for (
                var k = 2;
                k < data.body[i].length - 1;
                k += 2
            ) {
                dict[dict.length - 1]['chunks'][
                    j
                ][k / 2 - 1] = [
                    data.body[i][k],
                    data.body[i][k + 1],
    ...
    async parsed_to_db(
        ...
        try {
            ...
            Logger.error(
                'error on create nameYear and nameChanks',
                e,
            )
            throw e
        }
        ...
        try {
            ...
            Logger.error(
                'error on create excel fieldsofstudy',
                e,
            )
            throw e
        }
    ...
    async excel(file: Express.Multer.File) {
        ...
        var uploadedFile =
            await this.utilService.saveFile(file)

        // check, maybe we parsed?
        const check =
            await this.prisma.excelFieldsOfStudy.findFirst(
                {
                    where: {
                        originalFileChecksum:
