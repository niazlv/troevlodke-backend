...
export class ParseService {
    ...
    async parse_excel_file(file_url:Files/** prisma */) {
       ...
       var j = 0;
        for(var i = 0; i < data.body.length; i++) {
            ...
            for(var k = 2; k < data.body[i].length-1; k+=2) {
                dict[dict.length-1]['chunks'][j][(k/2)-1] = [data.body[i][k], data.body[i][k+1]];
            }
            dict[dict.length-1]['years'].push(data.body[i][data.body[i].length-1])
            j += 1;
    ...
    async parsed_to_db(data: {
            ...
            try{
                const schoolDB = await this.prisma.school.create({
                    data: {
                        ...
                        chunks: {"data":data.body[i].chunks},
                    }
                });
        ...
        var nameChunks = [];
        try {
            nameYear = data.header[data.header.length-2][data.header[data.header.length-2].length-1];
        ...
        try {
            ...
            //     }
            // });
            const excelFieldsOfStudy = await this.prisma.excelFieldsOfStudy.create({
                data: {
                    originalFileChecksum:file.checksum,
                    fileid:file.id,
                    schoolid:schoolid,
    ...
    async excel(file: Express.Multer.File) {
        ...
        var uploadedFile = await this.saveFile(file);
        ...
        const excelFieldsOfStudy = await this.parsed_to_db(parsed_file,uploadedFile)
